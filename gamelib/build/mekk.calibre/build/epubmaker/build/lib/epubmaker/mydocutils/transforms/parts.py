    ...
    def recurse (self, parent, follows_paragraph):
        ...
        for node in parent:
            ...
            if isinstance (node, (nodes.paragraph)):
                ...
                # invisible nodes are neutral, 
                # footnotes are not output here so they are neutral too
                pass
            else:
                # everything else
                follows_paragraph = False
...
#     def apply (self):
#         if not self.document.settings.no_images:
#             return
...
#     Has problems with inheritance. white-space-pre-line needs to be
#     inherited before this, but wrapping in inline elements happens
#     after this. Thus needing another inheritance step.

#     """

#     default_priority = 897

#     def apply (self, **kwargs):
#         for node in list (self.document.traverse (nodes.Text)):
#             if 'white-space-pre-line' in node.parent['classes']:
#                 text = node.astext ()
#                 if '\n' in text:
#                     newnodes = []
