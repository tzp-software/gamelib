...
TOC_HEADERS = (u'contents', u'table of contents',
               ...
               u'table de matieres', u'table de matières',
               u'inhoud')
    ...
    """   
        ...
        if hasattr (options.config, 'FILESDIR'):
            if source.startswith (options.config.FILESDIR):
                source = source.replace (options.config.FILESDIR, options.config.PGURL)
    ...
    def strip_pagenumbers (xhtml):
        ...
        for elem in xpath (xhtml, "//xhtml:*[@class]"):
            ...
            if STRIP_CLASSES.intersection (classes):
                ...
                # don't grok the XML minimized form of empty elements.
                # FIXME: We need a way to avoid outputting the
                # minimized form here.
